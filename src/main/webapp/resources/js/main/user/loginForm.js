...
window.onload = function() {
	...
	// status 경과가 진행중인 상태
	// state 한시점의 상태 고정된 현재 상태
	var loginState = '<c:out value="${param.loginResult}"/>';
	$('#scriptTest').text(loginState);
	...
	/*$("#rememberId").on("click", function() {
		...
		if($('#rememberId').prop("checked") == false) {
			$('#rememberId').prop("checked", true);
			// $('input:radio[name="rememberId"]').prop("checkd", true);
		}
